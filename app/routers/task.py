...
def create_task(task: schemas.Task, current_user: int = Depends(get_current_user), db: Session = Depends(get_db)):
    task_data = task.model_dump(exclude={'category'})
    new_task = models.Task(owner_id=current_user.id, **task_data)

    category = db.query(models.Category).filter(models.Category.name == task.category).first()
    if not category:
        category = models.Category(name=task.category)
        db.add(category)
        db.flush() 
...
def get_tasks(current_user: int = Depends(get_current_user), db: Session = Depends(get_db)):
    tasks = db.query(models.Task).outerjoin(models.Category).filter(models.Task.owner_id == current_user.id).all()
    return tasks
...
def get_task(id: int, current_user: int = Depends(get_current_user), db: Session = Depends(get_db)):
    task = db.query(models.Task).filter(models.Task.id == id).outerjoin(models.Category).first()
...
def update_task(id: int, updated_task: schemas.Task, current_user: int = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    category = db.query(models.Category).filter(models.Category.name == updated_task.category).first()
    if not category:
        category = models.Category(name=updated_task.category)
        db.add(category)
        db.flush()
    
    # Update task with excluded fields and set category
    task_data = updated_task.model_dump(exclude={'category'})
